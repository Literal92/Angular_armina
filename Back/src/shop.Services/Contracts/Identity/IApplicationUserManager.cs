...
namespace shop.Services.Contracts.Identity
{
    public interface IApplicationUserManager : IDisposable
    {
        ...
        Task<(List<User> list, int count, int totalPages)> SearchAdmin(int? id = null, string displayName = null, string userName = null, string mobile = null, int pageIndex = 1, int pageSize = 10, Expression<Func<User, object>>[] includes = null);

        User FindByRefererCode(string refererCode);
        User FindByUsername(string username);
        Task<User> GetVerifyCode(string username);
        Task<User> CooperationRequest(User model);
        Task UpdateUserLastActivityDateAsync(int userId);

        Task<string> GetSerialNumberAsync(int userId);

        Task<User> FindUserAsync(string username, string password);
        Task<User> FindUserAsync2(string username, string password);
        Task<User> FindBySerialNumber(string serialNumber);
        Task<IEnumerable<User>> GetUserAsync(Expression<Func<User, bool>> where = null,
       Func<IQueryable<User>, IOrderedQueryable<User>> orderby = null, int skip = 0, int take = 0,
       params Expression<Func<User, object>>[] includeProperties);
        Task<(List<User> Users, int Count, int TotalPages)> Get
            (int? id = null, int pageSize = 0, int pageIndex = 0, string Mobile = null, string displayName = null, int? status = null, Expression<Func<User, object>>[] includes = null);
        Task<IEnumerable<UserRole>> GetUserRoleAsync(Expression<Func<UserRole, bool>> where = null,
        ...
        Task<IdentityResult> UpdateUserAsync(User user, string newpass);
        Task<User> ChangePasswordAsync(User user, string newpass);
        Task<IdentityResult> AddUserRoleAsync(int userId, List<Role> roles);

        Task<int> Count(Expression<Func<User, bool>> where = null);
        //Task AddUserClaimsAsync(User user, List<UserClaim> claims);
        Task<IdentityResult> AddUserClaimsAsync(User user, List<UserClaim> claims);
        Task AddClaimAsync(User user, List<Claim> claims);
        Task<IdentityResult> Update(User user);

        Task<(HttpStatusCode status, bool success, string error)> UpdateActive(int id);
        Task<(HttpStatusCode status, bool success, string error)> ChangeTypeUser(int id);
        Task<(IdentityResult result, int id)> CreateWithRoleAsync(User user, string password, string role);
        Task<(List<User> list, int count, int totalPages)> Search(int? id = null, string displayName = null, string userName = null,
            string mobile = null, int pageIndex = 1, int pageSize = 10, Expression<Func<User, object>>[] includes = null);
        Task<User> GetByUserId(int id, Expression<Func<User, object>>[] includes = null);
        #endregion
    }
}