...
namespace shop.DataLayer.Context
{
    public interface IUnitOfWork : IDisposable
    {
       ...
       // Task<List<Provider>> CompileQuryProvider(List<int> ids);
        // Task<List<ReserveForUserRawReportViewModel>> ExecuteReaderAsync<T>(string sqlQuery) where T : class; 

        #region obsolete 
        DatabaseFacade db();
        IDbContextTransaction tran();
        Task<IDbContextTransaction> tranAsync();
