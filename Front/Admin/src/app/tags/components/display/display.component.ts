...
export class DisplayComponent implements OnInit, AfterViewInit {
  ...
  getPage(pageIndex: number, pageSize: number) {
    this.loading = true;
    this.pages = [];
    let model: iTagSearch = { pageIndex: pageIndex, pageSize: pageSize };
    this.service.get(model).subscribe(result => {
      console.log(result);
      let output = result as any;
      if (output != undefined) {
        this.tags = (output.pages != undefined) ? output.pages : [];
  ...
  SeoConfig() {
    // seo >> set title page
    this.seoService.SetTitle('مدیریت تگ ها');
    // seo >> set meta for page
    let meta = {
      charset: '', content: 'مدیریت تگ ها', httpEquiv: '', id: '', itemprop: '',
