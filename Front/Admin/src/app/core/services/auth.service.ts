...
export class AuthService {
  ...
  getAuthUser(): AuthUser | null {
    ...
    const roles = this.tokenStoreService.getDecodedTokenRoles();
    const roleClaims = this.tokenStoreService.getDecodeTokenRoleClaims();
    const userClaims = this.tokenStoreService.getDecodeTokenUserClaims();
  ...
  // #endregion
  // #region UserClaimRole
  isAuthUserInUserClaims(requiredClaim: ShemaUrl[]): boolean {
    const user = this.getAuthUser();
    if (!user || !user.roles) {
      return false;
    }
    ...
    return requiredClaim.some(requiredClaim => {
      if (user.userClaims) {
        let type = requiredClaim.area != null ?
          ...
          : `${(requiredClaim.controller).toLowerCase()}`;
        let value = (requiredClaim.action).toLowerCase();
        // return user.userClaims.indexOf({ type: type, value: value }) >= 0;
        //let output = user.userClaims.indexOf(t) >= 0;
        let index = user.userClaims.findIndex(x => x.type == type && x.value == value);
        return index > -1 ? true : false;
  ...
  isAuthUserInUserClaim(requiredClaim: ShemaUrl): boolean {
    return this.isAuthUserInUserClaims([requiredClaim]);
  }
