...
import { ShemaRoleClaimsToken, ShemaUserClaimsToken } from '../models/auth-guard-permission';
...
//# region  Decode RoleClaims
  getDecodeTokenRoleClaims(): any[] | null {
    const decodedToken = this.getDecodedAccessToken();
    const roleClaims = decodedToken["RoleClaims"];
    if (!roleClaims) {
      return null;
    }
    if (Array.isArray(roleClaims)) {

      return roleClaims.map(roleClaims => JSON.parse(roleClaims.toLowerCase()) as ShemaRoleClaimsToken);
    } else {
      return [JSON.parse(roleClaims.toLowerCase()) as ShemaRoleClaimsToken];
    }
  }

// #endregion

  //# region  Decode UserClaims
  getDecodeTokenUserClaims(): any[] | null {
    const decodedToken = this.getDecodedAccessToken();
    const userClaims = decodedToken["UserClaims"];
    if (!userClaims) {
      return null;
    }
    if (Array.isArray(userClaims)) {

      return userClaims.map(userClaims => JSON.parse(userClaims.toLowerCase()) as ShemaUserClaimsToken);
    } else {
      return [JSON.parse(userClaims.toLowerCase()) as ShemaUserClaimsToken];
    }
  }
